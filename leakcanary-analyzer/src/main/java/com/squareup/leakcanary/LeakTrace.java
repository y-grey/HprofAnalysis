...
public final class LeakTrace implements Serializable {
    ...
    public String leaks;
    public List<String> bmpAddress = new ArrayList<>();
    ...
    LeakTrace(List<LeakTraceElement> elements) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            String customDetailedString = element.toCustomDetailedString();
            if(customDetailedString.contains("(")){
                bmpAddress.add(customDetailedString.substring(customDetailedString.indexOf("(")+1,customDetailedString.length()-1));
            }
            if (i == 0) {
                GcRoot = element.toString();
            } else if (i == elements.size() - 1) {
                leaks = element.toString();
            }
        }
    }
